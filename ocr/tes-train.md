...
**Output:** `ocr/porsche.traineddata` — drop-in replacement for `eng.traineddata`.

## Pipeline

```
uv run ocr/generate.py   # generates ocr/train/ground-truth/ (tif + gt.txt + box)
uv run ocr/train.py      # downloads tessdata_best, generates lstmf, trains, packages
```
...
Output: `ocr/porsche.traineddata` (15 MB).
...
| `train/ground-truth/` | Generated tif + gt.txt + box files (gitignored) |
| `train/lstmf/` | Generated binary training data (gitignored) |
| `train/model/` | Checkpoints during training (gitignored) |
| `train/eng.traineddata` | Downloaded tessdata_best base model (gitignored) |
| `train/eng.lstm` | Extracted LSTM weights for fine-tuning (gitignored) |
| `porsche.traineddata` | Final packaged model (tracked) |
