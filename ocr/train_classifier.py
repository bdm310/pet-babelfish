...
def main():
    ...
    ap.add_argument("--no-report", action="store_true", help="skip writing report/onnx (for aug sweeps)")
    args = ap.parse_args()
    ...
    print(f"device={device} aug={not args.no_aug} hflip={args.hflip} vflip={args.vflip}", file=sys.stderr)
    ...
    results = {
        "val_cc": report_block(yva[va_cc], sva[va_cc], "val cc-only (996)"),
        "val_all": report_block(yva, sva, "val cc+gt (996)"),
        "test_cc": report_block(yte[te_cc], ste[te_cc], "TEST cc-only (997TT)"),
        "test_all": report_block(yte, ste, "TEST cc+gt (997TT)"),
    }
    ...
    # ---- dump full PR curves (val-cc, test-cc) for Phase 4 operating-point choice ----
    prec_t, rec_t, thr_t, _ = pr_curve(yte[te_cc], ste[te_cc])
    ...
    # operating points on TEST cc-only: highest precision achievable at each recall floor
    op_points = []
    ...
    # ---- FP/FN breakdown on TEST cc-only via manifest ----
    manifest = [json.loads(l) for l in open(MANIFEST, encoding="utf-8")]
    manifest = np.array(manifest, dtype=object)
    te_idx = np.where(te)[0]
    te_cc_idx = te_idx[te_cc]
    pred_te_cc = (ste[te_cc] >= thr).astype(np.int64)
    yt_cc = yte[te_cc]
    ...
    dummy = torch.from_numpy(Xte[:4]).to(device)
    torch.onnx.export(
        exp, dummy, ONNX_OUT,
    ...
    onnx_ok, max_diff = verify_onnx(exp, Xte[:64], device)
    ...
    meta = {
        ...
        "threshold_selected_on": "val cc-only (996 held-out sections), max-F1",
        "params": int(n_params),
        "onnx_opset": 13,
        "onnx_size_bytes": int(onnx_size),
        "test_cc": {k: results["test_cc"][k] for k in ("precision", "recall", "f1", "ap")},
    }
    json.dump(meta, open(META_OUT, "w"), indent=2)

    write_report(args, n_params, onnx_size, thr, best_epoch, results,
                 fp_sum, fn_sum, onnx_ok, max_diff, op_points)
...
def write_report(args, n_params, onnx_size, thr, best_epoch, R, fp, fn, onnx_ok, max_diff, op_points):
    def row(m):
    ...
    A("Binary classifier that gates candidate connected-component blobs into "
      ...
      "essentially all real callouts. Split is leave-one-catalog-out: "
      "**train/val = 996**, **test = 997-1Turbo-GT2**.\n")
    ...
    A(f"- ONNX vs PyTorch max abs diff on 64 test patches: **{max_diff:.2e}** (verified close)\n")
    ...
    A("|---|---|---|---|---|---|---|")
    for k in ("val_cc", "val_all", "test_cc", "test_all"):
        A(row(R[k]))
    A("")

    A("### Operating points — TEST cc-only (997TT)\n")
    A("The chosen max-F1 threshold is recall-heavy; since the gate's job is precision, "
      "Phase 4 may prefer a higher threshold. Best precision at each recall floor "
    ...
    A("## Confusion matrix — TEST cc-only (997TT), at chosen threshold\n")
    A("| | pred callout | pred not |")
    A("|---|---|---|")
    A(f"| **actual callout** | TP {tc['tp']} | FN {tc['fn']} |")
    A(f"| **actual not** | FP {tc['fp']} | TN {tc['tn']} |\n")

    A("## What are the errors? (TEST cc-only, via manifest bboxes)\n")
    ...
    A(f"- val (996) cc-only AUC-PR = **{vc['ap']:.4f}**, F1 = {vc['f1']:.4f}")
    A(f"- TEST (997TT) cc-only AUC-PR = **{tc['ap']:.4f}**, F1 = {tc['f1']:.4f}")
