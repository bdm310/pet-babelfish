  ...
  function create(db) {
    db.exec(`
      ...
      CREATE TABLE IF NOT EXISTS section (
        ...
        -- diagram_blob is the rendered diagram, per-diagram in one of two self-describing
        -- codecs: bilevel line art as a raw headerless CCITT Group 4 (ITU-T T.6) bitstream
        -- (~44% the size of lossy WebP), decoded by ccitt.js; greyscale/CAD renders as WebP
        -- (a hard threshold would destroy their shading), decoded natively. WebP's RIFF/WEBP
        -- magic distinguishes them; T.6 has no header, so diagram_w/diagram_h carry the pixel
        -- dimensions the G4 decoder needs. It lives in the DB because OPFS
